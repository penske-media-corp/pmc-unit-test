...
//			'jetpack/jetpack.php',
	...
	function pmc_deprecated_function( $function ) {
		...
		if ( in_array( $function, (array) $allowed_list, true ) ) {
			return false;  // ignore the deprecate warning/error message
		}
	...
	public function pmc_doing_it_wrong( $function ) {
		// Allowed list of the deprecates function to prevent unit test errors
		$allowed_list = [
	...
	public function init() {
		...
		// We should not send any headers during unit test
		// so disable all wp core header send to avoid unit test errors header already sent
		remove_all_actions( 'send_headers' );

		// we need to prevent shutdown events from trigger to prevent unit test errors during shutdown
		remove_all_actions( 'shutdown' );

		// workaround fix where do_action( 'init' ) trigger jetpack to reload causing fatal errors due to its
		// use of require instead of require_once, @see jetpack function jetpack_content_options_init fo details
	...
	/**
	 * We need to trigger a few events during the muplugins_loaded event with the highest priority
	...
	public function muplugins_loaded_early_bind() {
		...
		if ( function_exists( 'wpcom_vip_load_plugin' ) ) {
			// VIP Go Site
			// We need these constant defined to work properly for pmc global function plugin
		...
		if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
			...
			// We want to set out custom error handler to suppress php native deprecated warning message
			set_error_handler(
	...
	public function muplugins_loaded_late_bind() {
		...
		// We want to hook into wp core init function and run with a very low priority so we can override
		// some wp core events and remove filter/actions
	...
	public function load_pmc_required_plugins() : void {
		...
		// vip-init got loaded vi mu-plugins on vip go environment.
	...
	public function start( $options = [] ) {
		...
		// custom theme should be load during unit test
		...
		// This code is need for transition from themes/vip/pmc-theme to themes/pmc-theme structure for VIPGO sites
		if ( ! empty( $this->_theme )
		...
		// if no default is define, let define one if theme is given
