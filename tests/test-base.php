...
class Test_Base extends Base {
	...
	public function test_assertRedirect() {
		...
		$this->assert_redirect_to( '/test', function() {
			// Simulate code to trigger redirect with code 301
			wp_safe_redirect( '/test', 301 );
		}, 301 );

		$this->assert_not_redirect( function() {
	...
	public function test_traits() {
		...
		$this->assertEquals( $object, $GLOBALS['shortcode_tags']['test'] );
		$this->assertEquals( 'modified', $GLOBALS['shortcode_tags']['test']->value );
	...
	public function test_assert_hooks() {
		...
		$this->do_test_construct( Test_Construct::class, [
			[
				'name'     => 'filter1',
				'priority' => 10,
				'callback' => 'callable',
		...
		$this->do_test_construct( function() {
		...
		$filters = [
			...
			[
				'removed', 'shortcode', 'name' => 'remove_shortcode',
			],

			[
				'removed', 'name' => 'remove_all_filters', 'priority' => 10,
			],
			...
			'shortcode3'    => [ 'shortcode', 'callback' => 'test' ],
			'shortcode'     => [
					'test' => [ 'shortcode' ],
				],
			'shortcode1' => [
			...
			'filter1'    => [
				...
				'test5'      => 5,
				'test10'     => [ 'not' ],
				'test20'     => [ 'not', 'priority' => 20 ],
				'test100'    => [ 'priority' => 100 ],
				'function'   => [ 'function', 'not' ],
				'function10' => [ 'function' ],
				'function11' => [ 'function', 'priority' => 11 ],
				'function12' => [ 'function', 'priority' => 12, 'not' ],
