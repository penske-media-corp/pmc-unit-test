...
class Test_Utility extends Base {
	...
	public function setUp() {
		 ...
		 * when executing the cooresponding test of the same name.
		 */
		$per_test_url = $this->test_url . '?test=' . $this->getName();

		add_filter( $this->filter_name, function( $url ) use ( $per_test_url ) {
			return $per_test_url;
		} );
	}
	...
	public function test_code_coverage() {
		...
		$bufs = Utility::buffer_and_return(function(){
	...
	public function test_headers_list() {
		$headers = Utility::headers_list( function() {
			header( 'Header: Test' );
		} );
